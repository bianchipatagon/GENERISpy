...
for col, color in zip(cols, colors):
    # Plot positive part
    axs[2,0].bar(ALTGDM.index, pos_data[col], bottom=pos_bottoms,
                 color=color, alpha=0.8)
    pos_bottoms += pos_data[col].values

    # Plot negative part
    axs[2,0].bar(ALTGDM.index, neg_data[col], bottom=neg_bottoms,
                 color=color, alpha=0.8)
    neg_bottoms += neg_data[col].values
axs[2,0].xaxis.set_tick_params(labelsize=12, rotation=90)
axs[2,0].set_xlim(2022.5, 2050.5)
axs[2,0].axhline(linewidth=1, color='black', label='_nolegend_')
axs[2,0].set_ylim(-1500, 1500)
...
for col, color in zip(cols, colors):
    # Plot positive part
    axs[2,1].bar(ALTGDM.index, pos_data[col], bottom=pos_bottoms,
                 color=color, alpha=0.8)
    pos_bottoms += pos_data[col].values

    # Plot negative part
    axs[2,1].bar(ALTGDM.index, neg_data[col], bottom=neg_bottoms,
                 color=color, alpha=0.8)
    neg_bottoms += neg_data[col].values
axs[2,1].xaxis.set_tick_params(labelsize=12, rotation=90)
axs[2,1].set_xlim(2022.5, 2050.5)
...
for col, color in zip(cols, colors):
    # Plot positive part
    axs[2,2].bar(ALTGDM.index, pos_data[col], bottom=pos_bottoms,
                 color=color, alpha=0.8)
    pos_bottoms += pos_data[col].values

    # Plot negative part
    axs[2,2].bar(ALTGDM.index, neg_data[col], bottom=neg_bottoms,
                 color=color, alpha=0.8)
    neg_bottoms += neg_data[col].values
axs[2,2].xaxis.set_tick_params(labelsize=12, rotation=90)
axs[2,2].set_xlim(2022.5, 2050.5)
...
for col, color in zip(cols, colors):
    # Plot positive part
    axs[2,3].bar(ALTGDM.index, pos_data[col], bottom=pos_bottoms,
                 color=color, alpha=0.8)
    pos_bottoms += pos_data[col].values

    # Plot negative part
    axs[2,3].bar(ALTGDM.index, neg_data[col], bottom=neg_bottoms,
                 color=color, alpha=0.8)
    neg_bottoms += neg_data[col].values
axs[2,3].xaxis.set_tick_params(labelsize=12, rotation=90)
axs[2,3].set_xlim(2022.5, 2050.5)
axs[2,3].axhline(linewidth=1, color='black', label='_nolegend_')
'''
